...
router.get("/species/geo-search", async ctx => {
	...
	for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
		...
		if (
			speciesFound.has(speciesID) &&
			speciesFound.get(speciesID).distance === 0.0
		) {
			continue;
		}
		...
		if (distance <= searchRange) {
			// handles case where searchRange is 0.0
			// Species is within search range
			if (speciesFound.has(speciesID)) {
				speciesFound.get(speciesID).distance = Math.min(
					Math.round(distance),
					speciesFound.get(speciesID).distance,
				);
			} else {
