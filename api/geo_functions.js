...
const pointToPolygonDistance = function(testPoint, polygon) {
	...
	polygon.forEach(point2 => {
		...
		if (point2.pointIndex !== 0) {
			...
			if (
				point2.delta > testPoint.delta !==
					point1.delta > testPoint.delta &&
				testPoint.phi <
					((point1.phi - point2.phi) *
						(testPoint.delta - point2.delta)) /
	...
	if (inside) {
		...
		polygon.forEach(point2 => {
			...
			if (point2.pointIndex !== 0) {
				...
				const distanceToThisLine = pointToLineDistance(
					testPoint,
					point1,
					point2,
				);
				if (distanceToThisLine < distance) {
					// Math.min slower here, as always assigning?
...
const pointToLineDistance = function(point, linePoint1, linePoint2) {
	...
	const [vP, vL1, vL2] = [point, linePoint1, linePoint2].map(point =>
		geoToCartesian(point),
	);
	...
	if (dot1 > 0 !== dot2 > 0) {
		...
		if (Math.abs(dot1) < Math.abs(dot2)) {
			...
			distance =
				radiusOfEarthKm * Math.abs(Math.acos(dotProduct(vP, vL1)));
		} else {
			distance =
				radiusOfEarthKm * Math.abs(Math.acos(dotProduct(vP, vL2)));
		}
	}
